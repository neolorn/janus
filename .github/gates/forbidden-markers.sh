...
if markers=$(grep -rnE '\b(TODO|FIXME|HACK)\b' --include='*.cs' --include='*.csproj' --include='*.props' --include='*.sh' src tests); then
...
if commented=$(grep -rnE '^[[:space:]]*//[^/].*[;{}][[:space:]]*$' --include='*.cs' src tests); then
