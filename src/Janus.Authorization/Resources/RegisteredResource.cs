...
/// which organization owns it, and what contains it. The library never reads the
/// host's table, so the host says this when it creates or moves the record.
/// </summary>
/// <remarks>
...
internal sealed class RegisteredResource
{
    ...
    public static RegisteredResource Create(
        ...
        OrganizationId organization,
        ResourceReference? containedIn) =>
        new(reference, organization, containedIn);
